$(document).ready(function(){
     ...
     var drop = $('.pc');
     var basicdrop = $('.basic');
    basicdrop.click(function(){
        $(this).find('.option').toggleClass('on');
    });
    basicdrop.find('.option li').click(function(){
        var text = $(this).html();
        console.log(text);
        $(this).closest('.select-ph').children('p').html(text);
        $(this).closest('.select-ph').children('p').css('color','#333');
    });

     $(document).on('mouseup', function (e){
        var container = $(this).find('.option');
        if (!drop.is(e.target) && drop.has(e.target).length === 0 && !$(e.target).is(drop))
        {
    ...
    $(".last").click(function (e) {
        if ($(this).closest('.option').hasClass("on")){
            $(this).closest('.option').addClass("off")
            $(this).closest('.option').removeClass("on")
        }
        e.stopPropagation();
    });
